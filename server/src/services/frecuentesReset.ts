 ...
 * toda la señal), cada semana se redistribuye el conteo por ranking: el cliente
 * más visitado pasa a tener el valor más alto y el menos visitado 0.
 *
 *   antes:  a=50, b=37, c=12, d=2
 *   luego:  a=3,  b=2,  c=1,  d=0
 *
 * Así el orden relativo se conserva, los números se mantienen chicos
 * (acotados por la cantidad de clientes) y nunca se disparan por el uso.
 */
...
export function computeRedistribution(
  entries: { id: number; viewCount: number; lastViewedAt?: Date | string | null }[]
...
export async function runFrecuentesReset(): Promise<boolean> {
  ...
  const clients = await prisma.client.findMany({ select: { id: true, viewCount: true, lastViewedAt: true } });
  const next = computeRedistribution(clients);

  await prisma.$transaction(
    next.map((c) => prisma.client.update({ where: { id: c.id }, data: { viewCount: c.viewCount } }))
  );
...
export function scheduleFrecuentesReset(): void {
  const run = () => {
    runFrecuentesReset()
      .then((did) => {
        if (did) console.log("[frecuentes] contador redistribuido por ranking");
      })
