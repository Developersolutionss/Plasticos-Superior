...
import { buildExcelBuffer } from "../services/exportExcel";
...
exportRouter.get("/inventario", async (_req, res) => {
  ...
  const buffer = await buildExcelBuffer(
    ...
    stock.map((p) => ({ ...p, belowMinimo: p.belowMinimum ? "Sí" : "No" }))
  );
...
exportRouter.get("/facturas", requireVentas, async (_req, res) => {
  ...
  const rows = facturas.map((f) => {
    ...
    return {
      ...
      pagado,
      saldo: total - pagado,
      fecha: f.createdAt.toISOString().slice(0, 10),
    };
  });
  const buffer = await buildExcelBuffer(
    "Facturas",
...
exportRouter.get("/clientes", requireVentas, async (_req, res) => {
  ...
  const buffer = await buildExcelBuffer(
    ...
    [
      ...
      { header: "Límite de crédito", key: "limiteCredito" },
      { header: "Saldo pendiente", key: "saldoPendiente" },
    ],
    rows
  );
