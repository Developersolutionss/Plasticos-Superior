 ...
 * Pensada para verse en la pantalla de un celular apenas se escanea: se
 * refresca sola cada 5s, sin necesidad de tocar nada. */
export default function UbicacionDetalle() {
  const { code } = useParams<{ code: string }>();

  const { data, isLoading, isError } = useQuery({
    queryKey: ["warehouseLocationByCode", code],
    queryFn: () => api.getWarehouseLocationByCode(code!),
    enabled: !!code,
    refetchInterval: 5000,
  });

  if (isLoading) {
    return <p className="text-slate-500 text-center py-10">Cargando...</p>;
  }

  if (isError || !data) {
    return <p className="text-red-600 text-center py-10">No se encontró la ubicación "{code}".</p>;
  }
