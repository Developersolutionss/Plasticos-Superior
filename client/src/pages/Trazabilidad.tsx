  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div>
        ...
        <p className="text-sm text-slate-500">Historial completo de una orden de producción: estaciones, calidad y origen</p>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        <select
          className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-300 focus:border-slate-500"
          value={selectedId}
        ...
        <>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-100">
              <Package size={16} strokeWidth={2} className="text-slate-500" aria-hidden="true" />
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 text-sm">
              <div>
                <span className="block text-xs font-medium text-slate-500">Producto</span>
                <span className="text-slate-800">
                  {order.product.name} ({order.product.sku})
                </span>
              </div>
              <div>
                <span className="block text-xs font-medium text-slate-500">Cantidad planificada</span>
                <span className="text-slate-800">
                  {order.quantityPlanned} {order.product.unit}
                </span>
              </div>
              <div className="sm:col-span-2">
                <span className="block text-xs font-medium text-slate-500">Origen</span>
                <span className="text-slate-800">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-100">
              <Factory size={16} strokeWidth={2} className="text-slate-500" aria-hidden="true" />
              <p className="text-sm font-medium text-slate-700">Pasos por estación</p>
            </div>
            <ul className="divide-y divide-slate-100 text-sm">
              {order.stages?.map((s: any) => (
                <li key={s.id} className="px-5 py-3">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-slate-800">{STATION_LABELS[s.station] ?? s.station}</span>
              ...
              {(!order.stages || order.stages.length === 0) && (
                <li className="px-5 py-4 text-slate-500">Todavía no pasó por ninguna estación.</li>
              )}
            </ul>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-100">
              <CircleCheck size={16} strokeWidth={2} className="text-slate-500" aria-hidden="true" />
              <p className="text-sm font-medium text-slate-700">Control de calidad</p>
            </div>
            <div className="p-5 text-sm">
              {order.qualityCheck ? (
                <div className="flex items-start gap-2">
                  {order.qualityCheck.result === "aprobado" ? (
                    <CircleCheck size={18} strokeWidth={2} className="text-emerald-600 mt-0.5" aria-hidden="true" />
                  ) : (
                    <CircleX size={18} strokeWidth={2} className="text-red-600 mt-0.5" aria-hidden="true" />
                  )}
                  <div>
                    ...
                    {order.qualityCheck.observations && (
                      <p className="text-slate-600 mt-0.5">{order.qualityCheck.observations}</p>
                    )}
