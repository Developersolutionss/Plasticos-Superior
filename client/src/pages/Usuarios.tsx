...
const ROLE_LABELS: Record<string, string> = {
  ...
  operario_precorte: "Operario Precorte",
  calidad: "Calidad",
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-800 dark:text-slate-100">Usuarios y permisos</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Usuarios del sistema y su rol asignado</p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        {!isLoading && users?.length === 0 && <p className="p-4 text-center text-slate-500 dark:text-slate-400 text-sm">No hay usuarios.</p>}
              ...
              <tbody>
                {users.map((u: any) => (
                  <tr key={u.id} className="border-t border-slate-100 dark:border-slate-700">
                    ...
                    <td className="p-3 text-slate-600 dark:text-slate-300">{ROLE_LABELS[u.role] ?? u.role}</td>
                    <td className="p-3">
                      {u.active ? (
                      ...
                      {u.active ? (
                        <div className="flex items-center gap-2">
                          ...
                          {deactivatingId === u.id ? (
                            ...
                            <button className="text-red-600 dark:text-red-400 text-xs hover:underline" onClick={() => setDeactivatingId(u.id)}>
                              Desactivar
                            </button>
                          )}
                        </div>
                      ) : (
                        <button className="text-emerald-600 dark:text-emerald-400 text-xs hover:underline" onClick={() => handleReactivate(u.id)}>
                          Reactivar
                        </button>
                      )}
                    </td>
            ...
            <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700">
              {users.map((u: any) => (
                <div key={u.id} className="p-4 space-y-1.5">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">{ROLE_LABELS[u.role] ?? u.role}</p>
                  {u.active ? (
                    <div className="flex items-center gap-3 pt-1">
                      <button className="text-slate-600 dark:text-slate-300 text-xs hover:underline inline-flex items-center gap-1" onClick={() => setEditingUser(u)}>
                        <Pencil size={12} strokeWidth={2} /> Editar
                      </button>
                      {deactivatingId === u.id ? (
                        <>
                          <button className="text-red-600 dark:text-red-400 text-xs font-medium hover:underline" onClick={() => handleDeactivate(u.id)}>
                            Confirmar
                          </button>
                          <button className="text-slate-500 dark:text-slate-400 text-xs hover:underline" onClick={() => setDeactivatingId(null)}>
                            Cancelar
