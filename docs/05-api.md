...
1. **Salida**: `mode: "entrega"` = el operario de origen escanea y tipea a quién se lo entrega (`carrierName`, se guarda normalizado: espacios de más afuera y mayúscula inicial por palabra, así el mismo transportista no queda escrito de varias formas). `mode: "retiro"` = quien se lo lleva lo escanea con su cuenta (el `carrierName` enviado se ignora y queda el nombre de la cuenta). El servidor guarda `dispatchedKg`: el saldo real del rollo al salir.
2. **Recepción**: el operario de la bodega destino escanea el mismo QR cuando le llega. Un operario solo recibe en la bodega de su estación. Opcionalmente registra `receivedKg` (lo que marcó la balanza); si difiere de `dispatchedKg` más de 0,5 kg, se notifica a Gestión (`despacho_diferencia_peso`).

**Regla de consumo (Gestión, 2026-09-26)**: un rollo solo se consume en la estación donde está físicamente (`services/rollLocation.ts`): en la bodega del último despacho recibido, o en su estación de origen si nunca se movió. Para usarlo en otra estación primero hay que despacharlo y recibirlo allá. `POST /production-orders/:id/rolls` lo verifica dentro de la transacción, con el rollo bloqueado (el mismo lock que toma el despacho, así un despacho simultáneo no se cuela), y `GET /production-orders/rolls/by-code/:code?forStation=` avisa ya al escanear.
...
| GET | `/api/roll-transfers/carriers` | — | Nombres de transportistas ya usados (más recientes primero), para sugerirlos al despachar |
...
| POST | `/api/roll-transfers/:id/receive` | `{ code, token, notes?, receivedKg?, clientTimezone, clientUtcOffsetMinutes }` | Registra la recepción. 403 si el operario es de otra estación; 409 si ya se recibió. Con `receivedKg` distinto de `dispatchedKg` en más de 0,5 kg, avisa a Gestión |
