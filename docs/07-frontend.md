...
- **Carga en lote antes de confirmar**: "+ Añadir rollo" agrega la fila completada a una lista local **"por confirmar"** (`pendingRolls`), sin tocar el servidor todavía. Cada fila de esa lista se puede **editar** (ícono de lápiz: recarga sus valores en el formulario y la saca de la lista) o **borrar** (ícono de tacho) libremente.
- **"Confirmar N rollos"** (aparece solo si hay algo pendiente) manda todo el lote al servidor, una fila a la vez y en el orden en que se agregaron. Su tooltip avisa que después de confirmar ya no se puede editar, solo borrar. Si una fila falla a mitad del lote (ej. el servidor rechaza un peso), las que ya se confirmaron se sacan de la lista y las que quedan —incluida la que falló— se quedan pendientes para corregir y reintentar.
- Una vez confirmada, una fila se comporta como cualquier fila cargada de siempre: ya no se puede editar, solo **borrar** (`handleDeleteRoll`), y solo Gestión mientras la OP siga abierta.
- El peso/desperdicio de las filas pendientes ya cuenta contra la meta planificada (columna "Restan X kg") aunque todavía no se hayan confirmado.
- Cada fila pendiente con rollo madre guarda su propio saldo (`sourceRolls`), pero editar o borrar una fila no usa esa foto guardada: `rootSourceRollBalances` recupera el saldo original que devolvió el servidor (antes de que cualquier fila le sacara kilos) y `recomputeSourceRollBalances` simula el consumo de todas las filas pendientes, en orden, sobre ese saldo. `handleEditPendingRoll` y `handleDeletePendingRoll` usan este cálculo — con los ids de rollo madre de todas las filas pendientes, no solo los de la fila afectada, porque una fila puede repartir su peso entre dos rollos madre — para mostrar el saldo real de cada rollo madre al editar o borrar.
- Un rollo ya confirmado (guardado) solo se corrige borrándolo (`handleDeleteRoll`) y volviendo a cargarlo; no hay edición de peso/desperdicio/detalles sobre un rollo ya guardado. Una fila con un rollo madre asociado no se puede borrar si ya se le sacó material en otra estación.
- `deriveSpecsFromMeasure` (línea ~67) deriva los campos `ancho`, `calibre`, `medAncho` y `medLargo` del campo "Medidas" solo cuando el texto empieza con el patrón `número` o `número x número`; cualquier otro valor (vacío, símbolos, separador distinto de `x`/`X`) no deriva nada.
- El campo **"Material para"** (sección FORMA DEL MATERIAL, único campo que un operario puede guardar solo, vía `handleMaterialParaChange`) se guarda aparte y se refleja localmente en `specsDraft` sin recargar toda la OP; el guardado de ESPECIFICACIONES es independiente, con su propio botón.

**Escaneo (ScanDock)**: un solo botón flotante, abajo a la derecha, abre la cámara para escanear tanto el rollo madre como la etiqueta de bulto — el operario no elige el tipo de código.

- El sistema detecta el tipo por el patrón del código: una etiqueta de bulto siempre tiene el prefijo `EXT-` y 5 cifras con cero a la izquierda (`EXT-00001`, `padStart(5, "0")` en el servidor); un rollo real de Extrusión nunca tiene cero a la izquierda, porque su numeración por estación no usa relleno. Con esa diferencia, el cliente elige qué endpoint probar primero (`GET /production-orders/rolls/by-code` o `GET /bulto-labels/by-code`).
- Cuando el código tiene la forma inconfundible de etiqueta de bulto y esa búsqueda da 404, el cliente no reintenta como rollo — `ROLL_CODE_RE` del servidor no exige "sin cero a la izquierda", así que un rollo real con el mismo número sin padding matchearía por error. En sentido inverso (código de rollo con 404 → probar como etiqueta) sí se reintenta, porque ahí no hay riesgo de colisión: los códigos de etiqueta siempre llevan cero a la izquierda y se buscan por igualdad exacta de texto.
- `client/src/api/client.ts` lanza `ApiError` con el código HTTP, para distinguir un 404 real de otro error (401, 500).
- El aviso de lo escaneado queda fijo mientras el rollo o la etiqueta sigan seleccionados; no se borra solo, el botón "Quitar" lo saca. En PC queda junto al botón, abajo a la derecha. En celular aparece fijo arriba de la pantalla, como una notificación push, incluso con scroll.
- Los mensajes de error de `handleScanAny` muestran el texto crudo del último endpoint probado, lo que puede ser confuso si hubo un reintento de por medio. Un código que no matchea ningún formato conocido responde 400, no 404, así que el reintento — que solo dispara en 404 — no se activa para códigos basura.
