...
- El sistema detecta el tipo solo. Una etiqueta de bulto siempre tiene el prefijo `EXT-` y 5 cifras con cero a la izquierda (`EXT-00001`, generada con `padStart(5, "0")` en el servidor). Un rollo real de Extrusión nunca tiene cero a la izquierda, porque su numeración por estación no usa relleno. Con esa diferencia, el cliente elige qué endpoint probar primero (`GET /production-orders/rolls/by-code` o `GET /bulto-labels/by-code`) y prueba el otro si el primero responde 404.
...
Falta la regla equivalente del lado del servidor: hoy la detección de tipo es solo una regla del cliente, no hay un endpoint único que reciba un código y diga su tipo. Si una estación de Extrusión llega algún día a 10.000 rollos, un código de 5 cifras sin cero a la izquierda podría coincidir en número con una etiqueta de bulto ya usada — hoy no es un caso real.
