...
**Pendiente**: la tabla "Registro de rollos" (avance de la OP) se ve apretada en pantallas de celular — hay que revisar su diseño responsivo para que sea legible en móvil sin tener que hacer zoom o desplazar mucho horizontalmente.

**Pendiente**: hoy la única forma de corregir un rollo cargado por error es **borrarlo** (`handleDeleteRoll`, botón de tacho) y volver a cargarlo — no se puede editar un rollo ya guardado (peso, desperdicio, detalles). Falta una forma de corregir esos datos sin perder el rollo (por ejemplo, un modal de edición), sobre todo porque una fila con un rollo madre asociado no se puede borrar si ya se le sacó material en otra estación.
...
**Pendiente**: al crear una OP general (Extrusión) y cambiar el campo **"Material para"** (sección FORMA DEL MATERIAL, único campo que un operario puede guardar solo, vía `handleMaterialParaChange`, línea ~533), la llamada a `queryClient.invalidateQueries(["productionOrder", orderId])` dispara un refetch de `order`. El `useEffect` que reconstruye `specsDraft` a partir de `order.specs` (línea ~280) corre de nuevo y **pisa todo el borrador de ESPECIFICACIONES con lo último guardado en el servidor** — si el usuario ya había escrito datos en los campos de ESPECIFICACIONES sin guardarlos todavía (el guardado de esa sección es aparte, con su propio botón), esos datos se blanquean. Hay que evitar que ese refetch puntual pise el borrador local no guardado — por ejemplo, fusionando en vez de reemplazar, o excluyendo `materialPara` del re-render de todo `specsDraft`.
