...
- Se cargan **rollos**: formulario con los campos de la plantilla de la estación (`OP_TEMPLATES`), un botón único de escaneo (ScanDock, ver más abajo) para completar `sourceRollId` o la etiqueta de bulto, y vista previa del reparto de kilos entre rollos madre escaneados (`previewAllocation`, misma cuenta que hace el servidor) antes de guardar.
...
**Corregido**: dos mejoras sobre "Registro de rollos / avance" (`OrdenProduccionDetalle.tsx`):

1. **Campos automáticos vs. editables, más claros en celular.** En la tarjeta de carga de un rollo nuevo (`md:hidden`), los campos automáticos (FECHA, TURNO, OPERARIO, HORA, ETIQUETA, TOTAL...) no se distinguían de los campos que sí hay que llenar a mano — ambos se veían como texto plano. Ahora los automáticos llevan un ícono de candado (`Lock`, lucide) sobre fondo celeste; los editables usan un input real (`draftInput`: fondo blanco/oscuro sólido, borde visible, foco con anillo) en vez del estilo "hoja de papel" transparente (`sheetInput`) que usa el resto de la plantilla — intencional en Especificaciones (parece papel impreso), pero en la fila de carga no dejaba notar qué era tocable hasta tocarlo.

2. **Carga en lote antes de confirmar.** Antes, "+ Agregar fila" mandaba la fila al servidor de inmediato (quedaba guardada, solo se podía borrar). Ahora el flujo tiene dos pasos separados:
   - **"+ Añadir rollo"** agrega la fila completada a una lista local **"por confirmar"** (`pendingRolls`) — todavía no toca el servidor. Cada fila de esa lista se puede **editar** (ícono de lápiz: recarga sus valores en el formulario y la saca de la lista, para volver a añadirla corregida) o **borrar** (ícono de tacho) libremente, las veces que haga falta.
   - **"Confirmar N rollos"** (aparece solo si hay algo pendiente) manda todo el lote al servidor, una fila a la vez y en el orden en que se agregaron. Su `title` (tooltip al pasar el mouse) avisa que después de confirmar ya no se puede editar, solo borrar — no hay texto fijo en pantalla explicando esto, para no sobrecargar la tarjeta. Si una fila falla a mitad del lote (ej. el servidor rechaza un peso), las que ya se confirmaron se sacan de la lista y las que quedan —incluida la que falló— se quedan pendientes para corregir y reintentar, sin duplicar lo que ya se guardó.
   - Recién ahí, una vez confirmada, una fila se comporta exactamente como cualquier fila cargada de siempre (esto no cambió): ya no se puede editar, solo **borrar** (`handleDeleteRoll`), y solo Gestión mientras la OP siga abierta.
   - El peso/desperdicio de las filas pendientes ya cuenta contra la meta planificada (columna "Restan X kg") aunque todavía no se hayan confirmado, para no dejar cargar de más sin darse cuenta.

Verificado en el navegador con Playwright en viewport de celular (iPhone 13): se añadieron 2 rollos a la lista (10 kg y 20 kg), se editó uno (10 kg se cambió a 15 kg), se confirmaron ambos, y el total final quedó en 25 kg — igual en escritorio y celular (la tabla de escritorio también recibió la lista "por confirmar" y el botón de confirmar, con el mismo comportamiento).
...
**Pendiente QA**: se cambió el escaneo de "Registro de rollos / avance". Antes había dos bloques fijos en la hoja: uno para el rollo madre y otro para la etiqueta de bulto, cada uno con su propio botón. Ahora hay un solo botón flotante (ScanDock), abajo a la derecha:

- Un solo botón abre la cámara. El operario no elige el tipo de código.
- El sistema detecta el tipo solo. Una etiqueta de bulto siempre tiene el prefijo `EXT-` y 5 cifras con cero a la izquierda (`EXT-00001`, generada con `padStart(5, "0")` en el servidor). Un rollo real de Extrusión nunca tiene cero a la izquierda, porque su numeración por estación no usa relleno. Con esa diferencia, el cliente elige qué endpoint probar primero (`GET /production-orders/rolls/by-code` o `GET /bulto-labels/by-code`).
- **Corregido en QA**: cuando el código SÍ tiene la forma inconfundible de etiqueta de bulto (`EXT-` + 5 cifras con cero a la izquierda) y esa búsqueda da 404, el cliente ya NO reintenta como rollo. `ROLL_CODE_RE` del servidor no exige "sin cero a la izquierda", así que `EXT-00007` matchea igual el código de un rollo real (`Number("00007") === 7`) — sin este corte, una etiqueta mal tipeada, con el QR dañado o todavía sin imprimir (un 404 de lo más normal) terminaba enganchando en silencio el rollo de Extrusión con ese mismo número y descontándole kilos que no tenían nada que ver. Esto no era un caso hipotético de "algún día 10.000 rollos" — pasaba con cualquier 404 de etiqueta mientras existiera un rollo de Extrusión con el número correspondiente, que es el caso normal desde el primer día. En sentido inverso (código de rollo que da 404 → probar como etiqueta) no hay este riesgo: los códigos de etiqueta siempre llevan cero a la izquierda y se buscan por igualdad exacta de texto, así que un código de rollo nunca puede matchear una etiqueta real por error — ahí el reintento se mantiene.
- Para distinguir un 404 real de otro error (401, 500), `client/src/api/client.ts` ahora lanza `ApiError` con el código HTTP. Antes lanzaba un `Error` plano, sin ese dato.
- El aviso de lo escaneado queda fijo mientras el rollo o la etiqueta sigan seleccionados. No se borra solo. El botón "Quitar" lo saca.
- En PC, el aviso completo queda siempre junto al botón, abajo a la derecha.
- En celular, el aviso completo aparece fijo arriba de la pantalla, como una notificación push. Se queda arriba incluso si la persona hace scroll.

**Pendiente** (menor, no bloqueante): los mensajes de error de `handleScanAny` muestran el texto crudo del último endpoint probado, que puede ser confuso cuando hubo un reintento de por medio (ej. escanear un rollo real que no existe puede terminar mostrando "Etiqueta no encontrada" si además se reintentó como etiqueta). Y un código que no matchea ningún formato conocido responde 400 en vez de 404, así que el reintento — que solo dispara en 404 — nunca se activa para códigos basura aunque hubiera otro tipo disponible para probar.

Se probó con Playwright, en ventana visible, con una cámara virtual que lee un QR real (no un código escrito a mano) — en PC y en un viewport de celular (390×844). **Falta la verificación de una persona con un celular real**: la lectura del QR con la cámara física y con la luz de la planta, y que el aviso fijo de arriba no tape ningún botón importante de la barra superior de la app.
